...
extern BULLET bullets[80];
extern int score;
extern int time;
extern int highScore;


enum {BLACKID=(256-8), GREYID, MAROONID, GAMEBGID, GOLDID, BROWNID, SALMONID, PINKID};
...
# 21 "sonic.h"
extern const unsigned short sonicBitmap[96];


extern const unsigned short sonicPal[256];
# 8 "game.c" 2
...
void initGame() {
    ...
    initBullets();
    initVillain();
    unsigned short colors[8] = {(((0) & 31) | ((0) & 31) << 5 | ((0) & 31) << 10), (((5) & 31) | ((3) & 31) << 5 | ((3) & 31) << 10), (((18) & 31) | ((9) & 31) << 5 | ((9) & 31) << 10), (((18) & 31) | ((29) & 31) << 5 | ((31) & 31) << 10), (((26) & 31) | ((18) & 31) << 5 | ((1) & 31) << 10), (((10) & 31) | ((6) & 31) << 5 | ((5) & 31) << 10), (((23) & 31) | ((10) & 31) << 5 | ((10) & 31) << 10), (((27) & 31) | ((25) & 31) << 5 | ((25) & 31) << 10)};
    DMANow(3, sonicPal, ((unsigned short *)0x5000000), 256);
    score = 0;
    time = 0;
    for (int i = 0; i < 8; i++) {
...
void updatePlayer() {
    ...
    if ((~(buttons) & ((1<<6))) && player.y > 4) {
        player.dy = -2;
    } else if ((~(buttons) & ((1<<7))) && player.y + player.height < 155) {
        player.dy = 2;
    } else if ((~(buttons) & ((1<<4))) && player.x + player.width < 235) {
        player.dx = 2;
    } else if ((~(buttons) & ((1<<5))) && player.x > 4) {
        player.dx = -2;
    } else {
        player.dx = 0;
        player.dy = 0;
...
void updateBullet(BULLET *b) {
    if (b->active) {
        if (collision(b->x, b->y, b->width, b->height, player.x, player.y, player.width, player.height)) {
            player.lives--;
            player.x = 10;
            player.y = 80;
            b->active = 0;
